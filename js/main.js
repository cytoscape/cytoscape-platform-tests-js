...
function galfiltered(slide) {
  ...
  cyCaller.load_file_from_url(url, function (suid) {
    Logger.getInstance().log('Loaded galfiltered with SUID ::TTT::' + suid, slide.id)
    cyCaller.get('/v1/networks/' + suid + '/edges', function (edges) {
      edges = JSON.parse(edges)
      Logger.getInstannce().log('Edges in galfiltered = ' + edges.length, slide.id)
...
cyCaller.setLogCallBack(Logger.getInstance().log)
